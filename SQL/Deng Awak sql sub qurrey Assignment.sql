...
--Write a query that retrieves the order details for customers who have placed
--an order with a TotalAmount greater than $400. Use a subquery to find the
--customer IDs first, and then retrieve the orders from the Orders table.
SELECT * 
FROM Orders
WHERE CustomerID IN
...
SELECT 
    ...
    (SELECT COUNT(*) 
     FROM Orders AS O2 
     WHERE O2.CustomerID = O1.CustomerID) AS TotalOrdersByCustomer
FROM Orders AS O1;
